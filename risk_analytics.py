...
class BanCoppelRiskManager:
    ...
    def run_stress_test(
        ...
        job_id = "N/A"
        motor_nombre = "Desconocido"

        try:

            # =================================================
            # SIMULADOR (Fallback o Selección Manual)
            # =================================================

            if not use_quantum or backend is None:
                
                motor_nombre = "AerSimulator (Local)"
                return self.ejecutar_simulador(vector, motor_nombre)
            ...
            # En caso de error crítico en IBM, fallback a simulador local con aviso
            print(f"Error en ejecución cuántica: {e}. Fallback a simulador.")
            return self.ejecutar_simulador(vector, "AerSimulator (Fallback por Error)")
    ...
    def analisis_deep_audit(
        ...
        return {
            ...
            "Perdida_Esperada":
                float(prob_final * 10000) # Valor dummy para evitar errores en app.py si falta
        }
